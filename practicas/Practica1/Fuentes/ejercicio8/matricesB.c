 ...
 //Realiza la multiplicacion
  ...
  for(i=0;i<N;i++){
   for(j=0;j<N;j++){
    ...
    //double elem_c = 0;          --> aca ya habia probado lo del ejercicio D sin haber leido todavia el inciso
    C[i * N + j] = 0;
    for(k=0;k<N;k++){
      // Linea original 
	   //setValor(C,i,j,ORDENXFILAS, getValor(C,i,j,ORDENXFILAS) + getValor(A,i,k,ORDENXFILAS)*getValor(B,k,j,ORDENXFILAS));

      // Linea para acumular en una variable local
      //elem_c += elem_c + getValor(A,i,k,ORDENXFILAS)*getValor(B,j,k,ORDENXCOLUMNAS);

      // Linea que remueve los set y get para C
      C[i * N + j] += getValor(A,i,k,ORDENXFILAS)*getValor(B,j,k,ORDENXCOLUMNAS);
    }
   }
  }   
