 ...
 //Realiza la multiplicacion
  ...
  for(i=0;i<N;i++){
   for(j=0;j<N;j++){
    ...
    // Se modifica solo el contador en una variable local y se multiplican filas de A con columnas de B
    elemento = 0;
    for(k=0;k<N;k++){
	   //setValor(C,i,j,ORDENXFILAS, getValor(C,i,j,ORDENXFILAS) + getValor(A,i,k,ORDENXFILAS)*getValor(B,k,j,ORDENXFILAS));
      elemento += getValor(A,i,k,ORDENXFILAS)*getValor(B,j,k,ORDENXCOLUMNAS);
    }
    C[i * N + j] = elemento;
   }
  }   
