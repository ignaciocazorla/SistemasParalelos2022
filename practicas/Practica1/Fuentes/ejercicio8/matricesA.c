 ...
 //Inicializa las matrices A y B en 1, el resultado sera una matriz con todos sus valores en N
  for(i=0;i<N;i++){
   for(j=0;j<N;j++){
	...
	setValor(B,i,j,ORDENXFILAS,1);            // CUIDADO! si la matriz no se inicializase toda igual no 
                                             // podria intercambiar las columnas tan facil
   }
  }   
  ...
  timetick = dwalltime();
   ...
   for(i=0;i<N;i++){
      for(j=0;j<N;j++){
         ...
         //Se modifica para multiplicar filas de A por columnas de B
         //setValor(C,i,j,ORDENXFILAS, getValor(C,i,j,ORDENXFILAS) + getValor(A,i,k,ORDENXFILAS)*getValor(B,j,k,ORDENXCOLUMNAS));
         setValor(C,i,j,ORDENXFILAS, getValor(C,i,j,ORDENXFILAS) + getValor(A,i,k,ORDENXFILAS)*getValor(B,j,k,ORDENXCOLUMNAS));
     }
    }
   }   
