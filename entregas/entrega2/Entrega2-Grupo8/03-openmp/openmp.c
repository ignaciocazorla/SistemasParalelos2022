...
int main(int argc, char *argv[]){
  ...
  if ( (argc != 4) || ((n = atoi(argv[1])) <= 0) || ((bs = atoi(argv[2])) <= 0) || ((n % bs) != 0) || ((numThreads = atoi(argv[3])) % 2) != 0 )
  {
    printf("\nError en los parametros. Usage: ./%s N BS (N debe ser multiplo de BS)\n T (T tiene que ser multiplo de 2)", argv[0]);
    exit(1);
  }
  ...
  #pragma omp parallel shared(resultadosFib)
  ...
  #pragma omp parallel for private(i, j, valor) reduction(+:suma) reduction(max:max) reduction(min:min)
  for(i=0;i<n;i++){
    for(j=0;j<n;j++){ 
  ...
  int pos;
  #pragma omp parallel for shared(ABC, DporF, R) private(i, j, pos)
  for(i=0;i<n;i++){
    for(j=0;j<n;j++){ 
      pos = i*n + j;
